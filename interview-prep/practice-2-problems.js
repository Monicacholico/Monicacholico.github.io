...
const alreadySeen = arr.filter(num => {
    if(!seen.has(num)){
        seen.add(num);
        return true;  
    }
...
// Bonus: how would you combine two arrays and dedupe the result?
let arrayOne = [3, 4, 5, 6, 7, 0, 9]; let arrayTwo = [4, 5,3,5,4,63,64, 9];

const uniqueArray = new Set([...arrayOne, ...arrayTwo ]);

// console.log('expected [3,4,5,6,7,0,9,63,64]');
...
function twoSum(nums, target) {
  // YOUR CODE HERE
    ...
    for(let i = 0; i < nums.length; i++){
        ...
        map.set(nums[i], i);
        console.log('this is seen in twoSums:', map);
    }
}
...
function uniqueEmails(emails) {
  ...
  const formattedEmails = new Set();
  for( const email of emails){
    const [local, domain] = email.split('@');
    const cleanLocal = local.split('+')[0].replaceAll('.', '');
    formattedEmails.add(`${local}@${domain}`)
  }
