...
console.log(
    isValidAuthor({ firstName: "", middleName: "", lastName: "" }),
    "→ expected false",
);
...
function capitalize(str) {
    ...
    if (!str) return;
    const letterToCap = str.charAt(0).toUpperCase();
    const lastPiece = str.slice(1);
    console.log(letterToCap + lastPiece);
    return letterToCap + lastPiece;
}
...
function formatAuthor(author) {
    ...
    //   return fullName;
    if (author.block?.__typename === "Bold") {
        return `<strong>${fullName}</strong>`;
    } else if (author.block?.__typename === "Italics") {
...
console.log(formatAuthor({ random: "node" }), '→ expected ""');
...
function formatBylines(bylines) {
    ...
    // console.log("bylines", bylines.authors);

    const authors = bylines.authors || [];
    // console.log("authors", authors);

    const formattingAuthors = bylines.authors
        .filter((author) => isValidAuthor(author))
        .map((author) => formatAuthor(author));


    // console.log("format authors", formattingAuthors.join(" and "));

    if (formattingAuthors.length === 2) {
        return `By ${formattingAuthors.join(" and ")}`;
    } else if (formattingAuthors.length > 2) {
        const firstPart = formattingAuthors.slice(0, -1).join(", ");
        const lastAuthor = ` and ${formattingAuthors[formattingAuthors.length - 1]}`;
        return `By ${firstPart}${lastAuthor}`;
    }

    return `By ${formattingAuthors}`;
}
